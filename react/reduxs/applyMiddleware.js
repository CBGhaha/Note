export default function applyMiddleware(...middlewares) {
  ...
  return (createStore) => (reducer, preloadedState, enhancer) => {
    ...
    [next => action =>{ console.log(1);return next(action)},next => action =>{console.log(2);return next(action)}]
    ...
    //数组成员0执行之后的内容
    function (action){
      console.log(1);//这里的console.log可以是一些其他针对action的操作 比如如果action是函数 或则action是一个异步操作
